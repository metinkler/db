...
def main():
	...
	easyGames = ["Blind Hughie Dominoes", "Block Dominoes", "Chicken Foot", "Concentration Dominoes", "Cuban Dominoes"]
	interGames = ["All Fives dominoes", "All Threes Dominoes", "Bergen", "Billitonnen"]
	hardGames = ["Auction Draw Dominoes", "Austrian Dominoes", "Buki Dominoes", "Cut Throat Dominoes Jamaican"]
...
def produceRates(easyGames, interGames, hardGames, easyDict, interDict, hardDict, M, priorProbEasy, priorProbInter, priorProbHard, cur):
	cur.execute("SELECT name FROM domino ORDER BY name;")
	...
	for game in allGames:
		...
		game = "'" + game + "'"
		if maxSum == sumEasy:
			cur.execute("UPDATE domino SET complexity = 'easy' WHERE name = %s;" % (game))
			print("%s: easy" % (game))
		elif maxSum == sumInter:
			cur.execute("UPDATE domino SET complexity = 'intermediate' WHERE name = %s;" % (game))
			print("%s: intermediate" % (game))
		else:
			cur.execute("UPDATE domino SET complexity = 'hard' WHERE name = %s;" % (game))
			print("%s: hard" % (game))
...
def classifyGame(game, easyDict, interDict, hardDict, M, priorProbEasy, priorProbInter, priorProbHard, cur):
	...
	cur.execute("SELECT synopsis FROM domino WHERE name = '%s';" % (game))
